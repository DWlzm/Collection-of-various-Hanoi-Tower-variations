...
void hanoi_red_blue(int n,char from ,char temp,char to){
    if(n==1){           // from到to只有一个需要移动就直接移动 
        move_red_blue(from, to); //递归截止条件
        cnt1++;
    }
    else{
        hanoi_red_blue(n-1,from ,to,temp); //移动from 上面的n-1 ==> temp 暂存 
        move_red_blue(from,to);            //移动from 剩下的1   ==> to   目的地 
        cnt1++;
        hanoi_red_blue(n-1,temp,from,to);  //移动temp 暂存的n-1 ==> to   目的地 
    }
}
